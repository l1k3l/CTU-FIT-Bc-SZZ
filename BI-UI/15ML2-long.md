...
**Odhad — matice návrhu (design matrix) $\boldsymbol\Phi$.** Pro trénovací množinu $(Y_1,x_1),\dots,(Y_N,x_N)$ zavedeme **matici návrhu / bázovou matici** $\boldsymbol\Phi\in\mathbb R^{N,M}$ ($N$ řádků = trénovací body, $M$ sloupců = bázové funkce):
$$\boldsymbol\Phi=\begin{pmatrix}\varphi(x_1)^T\\ \vdots\\ \varphi(x_N)^T\end{pmatrix}=\begin{pmatrix}\varphi_1(x_1)&\cdots&\varphi_M(x_1)\\ \vdots&\ddots&\vdots\\ \varphi_1(x_N)&\cdots&\varphi_M(x_N)\end{pmatrix},\qquad \boldsymbol\Phi_{i,j}=\varphi_j(x_i).$$
Tedy **$i$-tý řádek je transformovaný $i$-tý trénovací bod** $\varphi(x_i)^T$ a **$j$-tý sloupec je $j$-tá bázová funkce** vyčíslená ve všech bodech. Pak $\boldsymbol Y=\boldsymbol\Phi w+\varepsilon$. Minimalizujeme regularizovaný RSS
...
- **Starosta (léto 2024):** „Jak vypadá matice bázových funkcí — její rozměr a prvky?" (= matice návrhu / design matrix $\boldsymbol\Phi$: $N\times M$, $\boldsymbol\Phi_{i,j}=\varphi_j(x_i)$; řádky = transformované trénovací body $\varphi(x_i)^T$, sloupce = bázové funkce) → §2
- **Starosta (léto 2024):** „Popište duální reprezentaci." (substituce $w=\boldsymbol\Phi^T\alpha$ → vše přes Gramovu/jádrovou matici; $\hat\alpha=(G+\lambda I)^{-1}\boldsymbol Y$) → §3
- **Holeňa (komise, předseda) (LS 2021):** zájem o bázové funkce + jádrovou funkci (jádrový trik = nahrazení skalárních součinů $\varphi(x)^T\varphi(y)$ funkcí $k$) → §2, §4
