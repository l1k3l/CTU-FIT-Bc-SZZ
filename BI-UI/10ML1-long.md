...
- Koeficient $w_0$ se nazývá **intercept** a odpovídá (očekávané) výchozí hodnotě $Y$ při nulových příznacích (geometricky: posouvá regresní nadrovinu — bez interceptu by musela procházet počátkem).
- **$\varepsilon$ je náhodná veličina** (ne konstanta) — shrnuje neznámé vlivy + šum v datech. Předpoklad $\mathrm{E}\,\varepsilon = 0$ zajišťuje, že chyba je v průměru vystředěná, tj. model nemá **systematickou odchylku** (bias); kdyby $\mathrm{E}\,\varepsilon \neq 0$, dala by se nenulová střední hodnota „schovat“ právě do interceptu $w_0$.

> *Doplnění nad rámec slidů:* slidy u $\varepsilon$ uvádějí jen, že jde o náhodnou veličinu s $\mathrm{E}\,\varepsilon = 0$, a trénovací data berou jako **náhodný výběr** z modelu. Standardní (a u zkoušky vyžadovaný) předpoklad navíc je, že chyby $\varepsilon_i$ jsou **nezávislé stejně rozdělené** (i.i.d.) a že $\varepsilon$ je **nejčastěji z normálního (Gaussova) rozdělení**, $\varepsilon \sim N(0,\sigma^2)$ (homoskedasticita = konstantní rozptyl $\sigma^2$). Za této pravděpodobnostní interpretace je pak $Y \mid x \sim N(w^Tx, \sigma^2)$ a MNČ odhad $\hat w_{\mathrm{OLS}}$ splývá s **odhadem metodou maximální věrohodnosti** (MLE).
...
**Rozměry (kontrola konzistence — bývá doptáváno):** $\mathbf{X} \in \mathbb{R}^{N,p+1}$, takže $\mathbf{X}^T \in \mathbb{R}^{p+1,N}$, $\mathbf{Y} \in \mathbb{R}^{N}$ (sloupec $N\times 1$), $w \in \mathbb{R}^{p+1}$. Pak $\mathbf{X}^T\mathbf{X} \in \mathbb{R}^{p+1,p+1}$ (čtvercová!), $\mathbf{X}^T\mathbf{Y} \in \mathbb{R}^{p+1}$ a $\mathbf{X}^T\mathbf{X}\,w \in \mathbb{R}^{p+1}$ — rozměry obou stran sedí. Transpozice u $\mathbf{X}^T$ je nutná právě proto, aby vnitřní rozměry $(p+1)\times N$ a $N\times(p+1)$ navazovaly.
...
Inverze $\mathbf{X}^T\mathbf{X}$ (viz [[Inverzní-matice|inverzní matice]]) pak sice teoreticky existuje (matice je regulární), ale její výpočet je **numericky problematický** — matice je **špatně podmíněná**.

> *Doplnění nad rámec slidů:* slidy říkají jen, že inverze je „numericky problematická / nestabilní“. Kvantitativní mírou je **číslo podmíněnosti** $\kappa(\mathbf{X}^T\mathbf{X})$ (poměr největšího a nejmenšího vlastního čísla); pro kolineární data je obrovské. Numericky proto bývá lepší řešit MNČ přes [[QR-rozklad|QR rozklad]] nebo [[SVD]] než přes normální rovnice, protože $\kappa(\mathbf{X}^T\mathbf{X}) = \kappa(\mathbf{X})^2$ (sestavení $\mathbf{X}^T\mathbf{X}$ podmíněnost umocní na druhou).
...
> ⚠️ **NEJŽHAVĚJŠÍ ML1 otázka** — v 2025 zkoušeli DVA členové komise (Petr i Dedecius), oba dali A, oba se vrtali hlavně do **pravděpodobnostní interpretace**. Tu umět nazpaměť.

- **Petr (komise, 2025):** chtěl **přesnou definici RSS** — student ji nejdřív napsal špatně, examinátor doptal a nechal opravit. Pozor na tvar $\mathrm{RSS}(w) = \sum_i (Y_i - w^Tx_i)^2 = \|\mathbf{Y}-\mathbf{X}w\|^2$ (rozdíl skutečnost − predikce, na druhou, suma přes všechny body). → §2.1
- **Petr (komise, 2025):** **pravděpodobnostní interpretace** — chtěl slyšet termíny **„náhodný výběr“** (trénovací data) a **„náhodné rozdělení“** chyby $\varepsilon$. → §1.2, §1.3
- **Dedecius (komise, léto 2025, BI-UI.21):** zúžení zadání — „popište model, ukažte příklad použití, vyberte a popište JEDNU metodu hledání vah“. Doptával hlavně pravděpodobnostní část: **k čemu je intercept**; **co je $\varepsilon$** (= náhodná veličina); **proč chceme $\mathrm{E}\,\varepsilon = 0$**; a **z jakého rozdělení je $\varepsilon$ nejčastěji — odpověď: NORMÁLNÍ (Gaussovo)** $\varepsilon \sim N(0,\sigma^2)$. → §1.2
- **Dedecius (komise, léto 2025):** geometricky **co dělá intercept** (posun nadroviny mimo počátek). Chválil: odvození $\hat w_{\mathrm{OLS}}$ z **normální rovnice**, **důkaz PSD Hessovy matice RSS** ($s^T\mathbf{X}^T\mathbf{X}s = \|\mathbf{X}s\|^2 \ge 0$), **Mooreovu–Penroseovu pseudoinverzi** a diskusi **počtu řešení** (1 / ∞). → §1.2, §2.3, §2.4, §4.2
- **Klouda (mimo komisi, LS2020):** u geometrické interpretace doptával **intercept**, **$\mathrm{E}\,\varepsilon$**, **inverzní matici** a rozdíl **kolinearita vs. lineární závislost** (přesná vs. „skoro“); nejvíc ho zajímal **OBRÁZEK** geometrické interpretace (Y, projekce $\mathbf{X}w$, kolmé reziduum) + lineární algebra za ním. → §3, §4.1, §4.3
- **Vašata (mimo komisi, léto 2020):** pedantsky doptával **rozměry matic/vektorů** v normální rovnici a **transpozice** ($\mathbf{X}^T$). → §2.3
