...
Výhody: **libovolný (nekonvexní) tvar shluků**, **detekce šumu** (odolnost k odlehlým bodům), **nezadává se $k$**. Nevýhody: citlivost na **$\varepsilon$** (MinPts méně důležitý; MinPts $\approx 4$–$6$), potíže s různou hustotou shluků.
...
- **k-means:** minimalizuj WCSS $\sum_i\sum_{x\in C_i}\|x-\bar{x}_i\|^2$; Lloyd = přiřaď k nejbližšímu centroidu → přepočti centroidy; lokální min, zadej $k$, sférické shluky, k-means++.
